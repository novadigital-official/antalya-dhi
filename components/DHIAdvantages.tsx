      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {advantages.map((item) => {
            ...
            return (
              <div key={item.id} className="bg-white border border-slate-200 rounded-3xl p-6 shadow-xs flex flex-col justify-between card-hover-soft">
                <div>
                  <div className="w-10 h-10 rounded-2xl bg-emerald-50 text-emerald-700 flex items-center justify-center mb-4 border border-emerald-200/80">
                    <Icon className="w-5 h-5" />
                  </div>
                  <h3 className="font-heading font-bold text-sm sm:text-base text-slate-900 mb-1.5 leading-snug">
                    {item.title}
                  </h3>
